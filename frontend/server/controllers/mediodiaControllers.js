...
const insertarMediodia = (req, res) => {
    ...
    conecction.query('INSERT INTO mediodia (id_medic, horamediodia, fecha, estatus) VALUES (?, ?, ?, ?)', [id_medic, hora, fecha, estatus], (error, results) =>{
        if(error){
...
const eliminarMediodia = (req, res) =>{
    ...
    conecction.query('DELETE FROM mediodia WHERE id_mediodia = ?', [id], (error, results) =>{
        if(error){
